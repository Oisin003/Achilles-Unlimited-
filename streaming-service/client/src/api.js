...
export const api = {
  ...
  getSeasonNames: (showId) => http("GET", `/api/admin/shows/${showId}/season-names`, null, { admin: true }),
  setSeasonName: (showId, seasonNumber, name) => http("PUT", `/api/admin/shows/${showId}/season-names/${seasonNumber}`, { name }, { admin: true }),
  ...
  getDocumentChapters: (id) => http("GET", `/api/documents/${id}/chapters`),
  getDocumentChapter: (id, chapterNumber) => http("GET", `/api/documents/${id}/chapters/${chapterNumber}`),
  ...
  adminListUsers: () => http("GET", "/api/admin/users", null, { admin: true }),
  adminDeleteUser: (id) => http("DELETE", `/api/admin/users/${id}`, null, { admin: true }),

  // Music uploads (user)
  uploadMusicAudio: async (file, onProgress) => {
    const form = new FormData();
    form.append("file", file);
    return uploadWithProgress("/api/music/upload/audio", form, onProgress);
  },

  uploadMusicCover: async (file, onProgress) => {
    const form = new FormData();
    form.append("file", file);
    return uploadWithProgress("/api/music/upload/cover", form, onProgress);
  },

  createMusic: (data) => http("POST", "/api/music", data),
  listUserMusic: (userId) => http("GET", `/api/music/user/${userId}`),

  // Admin - Music Management
  adminListMusic: () => http("GET", "/api/admin/music", null, { admin: true }),
  adminDeleteMusic: (id) => http("DELETE", `/api/admin/music/${id}`, null, { admin: true }),
  ...
  adminUploadTrailer: (file, onProgress) => {
    const formData = new FormData();
    formData.append("file", file);
    return uploadWithProgress("/api/admin/upload/trailer", formData, onProgress, { admin: true });
  },
  adminSaveTrailer: (data) => http("POST", "/api/admin/trailers", data, { admin: true }),
  adminDeleteTrailer: (movieId) => http("DELETE", `/api/admin/trailers/${movieId}`, null, { admin: true })
};
