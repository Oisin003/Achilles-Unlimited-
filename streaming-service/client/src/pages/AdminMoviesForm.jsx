...
export default function AdminMovieForm({ mode }) {
  ...
  async function uploadTrailer(file) {
    ...
    setErr("");
    try {
      const res = await api.adminUploadTrailer(file, (progress) => {
        setTrailerProgress(progress);
      });
      setTrailerPath(res.trailerPath);
    } catch (e) {
      setErr(e.message);
    } finally {
  ...
  async function save(e) {
    ...
    try {
      let movieId = id;
      if (mode === "create") {
        const created = await api.adminCreateMovie(payload);
        movieId = created.id;
      } else {
        await api.adminUpdateMovie(id, payload);
      }

      // Save or update trailer if provided
      if (trailerPath) {
        await api.adminSaveTrailer({
          movieId: movieId,
    ...
    <div className="panel formCard">
      <div className="panelBody">
        ...
        <form className="form" onSubmit={save} style={{ marginTop: 8 }}>
          ...
          <label>Trailer (upload video)</label>
          <input type="file" accept="video/*" onChange={e => e.target.files?.[0] && uploadTrailer(e.target.files[0])} />
          {uploadingTrailer && (
            <div style={{ marginTop: 8 }}>
              <div className="subtle">Uploading trailer… {trailerProgress}%</div>
              <div style={{
                height: 4,
                background: "rgba(255,255,255,0.1)",
                borderRadius: 2,
          ...
          <input value={trailerPath} onChange={e => setTrailerPath(e.target.value)} placeholder="Set after upload" />

          {trailerPath && (
            <div style={{ marginTop: 16 }}>
              <div className="subtle" style={{ marginBottom: 8 }}>Trailer Preview:</div>
              <video controls style={{ width: '100%', maxWidth: 600, borderRadius: 8 }}>
                <source src={fileUrl(trailerPath)} />
              </video>
            </div>
          )}
